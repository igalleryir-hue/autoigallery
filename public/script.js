...
const demoCars = [
  { demo:true, id:'demo1', title:'کیا K5 GT لاین', brand:'کیا', model:'K5', year:2023, mileage:12000, price:2918000000, transmission:'اتوماتیک', body_status:'بدون رنگ', seller_phone:'09123060749', image:'/assets/car-sample-1.jpg', description:'نمونه چیدمان ویترین؛ برای نمایش خودروهای واقعی، از پنل مدیریت یا پنل کاربر آگهی تاییدشده اضافه کنید.' },
  { demo:true, id:'demo2', title:'هیوندای سانتافه', brand:'هیوندای', model:'سانتافه', year:2022, mileage:24000, price:3450000000, transmission:'اتوماتیک', body_status:'بدون رنگ', seller_phone:'09123060749', image:'/assets/car-sample-2.jpg', description:'نمونه چیدمان ویترین؛ برای نمایش خودروهای واقعی، از پنل مدیریت یا پنل کاربر آگهی تاییدشده اضافه کنید.' },
  { demo:true, id:'demo3', title:'تویوتا کمری هیبرید', brand:'تویوتا', model:'کمری', year:2021, mileage:16000, price:3020000000, transmission:'اتوماتیک', body_status:'بدون رنگ', seller_phone:'09123060749', image:'/assets/car-sample-3.jpg', description:'نمونه چیدمان ویترین؛ برای نمایش خودروهای واقعی، از پنل مدیریت یا پنل کاربر آگهی تاییدشده اضافه کنید.' },
  { demo:true, id:'demo4', title:'بنز C200', brand:'بنز', model:'C200', year:2020, mileage:23000, price:4780000000, transmission:'اتوماتیک', body_status:'بدون رنگ', seller_phone:'09123060749', image:'/assets/car-sample-4.jpg', description:'نمونه چیدمان ویترین؛ برای نمایش خودروهای واقعی، از پنل مدیریت یا پنل کاربر آگهی تاییدشده اضافه کنید.' }
];
...
function carCard(car, mine=false){
  ...
  const canOpen = isPublic && !car.demo;
  ...
  return `<article class="productCard upgradedCarCard">
    <button class="thumb productThumb" ${canOpen ? `onclick="showCar(${car.id})"` : ''} aria-label="مشاهده ${escapeHtml(car.title)}">
    ...
    <div class="productBody">
      ...
      <button class="titleBtn" ${canOpen ? `onclick="showCar(${car.id})"` : ''}><h3>${escapeHtml(car.title)}</h3></button>
      ...
      <div class="cardActions">
        ${car.demo ? `<span class="msg demoMsg">نمونه چیدمان</span><a class="btn ghost small" href="tel:${car.seller_phone}">تماس</a>` : (isPublic ? `<button class="btn small" onclick="showCar(${car.id})">جزئیات و تماس</button><a class="btn ghost small" href="tel:${car.seller_phone}">تماس</a>` : `<span class="msg">${statusText(car.status)}</span>`)}
...
async function loadCars(){
  ...
  if (Array.isArray(cars) && cars.length === 0) cars = demoCars;
...
async function userRegister(){
  ...
  const res = await fetch('/api/user/register', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) });
  const data = await res.json();
  if(!res.ok) return setMsg('userRegisterMsg', data.error || 'ثبت‌نام انجام نشد.');
  localStorage.setItem(userTokenKey, data.token);
  setMsg('userRegisterMsg', data.message || 'ثبت‌نام انجام شد.');
  userInit();
}
async function userLogin(){
  setMsg('userLoginMsg','');
  const res = await fetch('/api/user/login', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ phone:$('userLoginPhone').value }) });
  const data = await res.json();
  if(!res.ok) return setMsg('userLoginMsg', data.error || 'ابتدا ثبت‌نام کنید.');
  localStorage.setItem(userTokenKey, data.token);
  setMsg('userLoginMsg', data.message || 'ورود انجام شد.');
  userInit();
}

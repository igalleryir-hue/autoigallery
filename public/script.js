...
async function userRegister(){
  ...
  const payload = { first_name:$('userFirstName').value, last_name:$('userLastName').value, phone:$('userRegisterPhone').value };
  const res = await fetch('/api/user/register', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) });
  const data = await res.json();
  if(!res.ok) return setMsg('userRegisterMsg', data.error || 'خطا در ثبت‌نام.');
